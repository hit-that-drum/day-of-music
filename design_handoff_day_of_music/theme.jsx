// theme.jsx — Visual theme tokens for Day of Music.
// Four aesthetic directions × four typography pairs. Tokens are applied as CSS
// custom properties on the root frame so the whole app can theme uniformly.
...
function ensureFonts() {
  ...
  const href = 'https://fonts.googleapis.com/css2?' + Array.from(all).map(f => `family=${f}`).join('&') + '&display=swap';
  const link = document.createElement('link');
  link.id = 'dom-fonts'; link.rel = 'stylesheet'; link.href = href;
...
function applyTheme(rootEl, aestheticKey, typeKey) {
  if (!rootEl) return;
  ...
  Object.entries(a.tokens).forEach(([k, v]) => {
...
Object.assign(window, { AESTHETICS, TYPE_PAIRS, ensureFonts, applyTheme });
